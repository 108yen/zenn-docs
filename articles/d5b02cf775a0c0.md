...
::::message
App Routerで`layout.tsx`に`use client`をつけたくない場合は、`ThemeSchemeScript`と`ColorModeScript`をクライアントコンポーネントとしてまとめて下さい。

:::details sample

```tsx:yamada-ui-scripts.tsx
"use client"
...
```tsx:layout.tsx
import { YamadaUIScripts } from "./yamada-ui-scripts"
